...
DROP TRIGGER IF EXISTS UPDATE_ON_MONEY;

USE DELIMITER $$
CREATE TRIGGER INPUT_IN_MONEY
AFTER INSERT ON `GSCookieDB`.`PRODUCT_ORDER`
FOR EACH ROW BEGIN
    ...
    SELECT TROOP_ID AS SCOUT_TROOP INTO @SCOUT_TROOP FROM SCOUT WHERE `SCOUT`.`TROOP_ID` = NEW.SCOUT_ID;
...
USE DELIMITER $$
CREATE TRIGGER UPDATE_ON_MONEY
AFTER UPDATE ON `GSCookieDB`.`MONEY`
FOR EACH ROW BEGIN
